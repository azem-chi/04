...
const SHARED_GROQ_KEY = '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';
...
function getDaySchedule(day) {
  ...
  if (S.user?.program === 'beginner') {
    ...
    const exercises = exIds.map(id => {
      ...
      if (!ex) return null;
      // Beginner: Smart overload مع تخفيف أولي
      const beLoad = calcSmartOverload(ex, week, day);
      return { ...ex,
        sets: Math.max(2, beLoad.sets - 1),
        reps: Math.max(5, Math.round(beLoad.reps * 0.75)) };
    }).filter(Boolean);
    return { ...ws, exercises };
  }
  ...
  const exercises = exIds.map(id => {
    const ex = allEx.find(e => e.id === id);
    if (!ex) return null;
    // Smart Progressive Overload — يحلل الأداء الفعلي
